...
/**
 * 
 * NOTE:Though I am not allowing users to delete specific applcation, because we want to track
 * the number of applications applied, rejected, interview etc for long time.
 * 
 * IF user wants to start fresh, her can go erase all data from info Modal.
 * 
 * We are not using this delete method anywhere, even though it exists.
 */
  delete(link: string) {
    try {
      let prevNode = null;
